...
stage2_dialog = Dialog(
    ...
    Window(
        Const(
            ...
            "Поздравляем с прохождением первого этапа! Впереди тебя ждут задания отбора.",
            when="can_start"
        ),
        Const(_ALREADY_DONE_TEXT, when="already_completed"),
        Const(_NO_STAGE1_TEXT, when="no_stage1"),
        Button(
            Const("➡️ Начать 2-й этап"), # TODO: 
            id="btn_stage2_start",
    ...
    Window(
        Format(
            ...
            "{num_emoji_1}  Тестирование состоит из <b>3 развернутых письменных вопросов</b> и <b>5 вопросов видеоинтервью</b>.\n"
            "{num_emoji_2}  Правила таймера: <b>{duration_str}</b>.\n"
            "{num_emoji_3}  Видеоинтервью записывается в формате <b>видео-кружочков</b> в Telegram (длительность до 1 мин).\n\n"
            "{exclamation_emoji}  В общем тестирование займет не более <b>25 минут</b>. Обязательно убедись, что у тебя есть возможность закончить тестирование за один раз.\n\n"
    ...
    Window(
        Format(
            f"<b>{emoji("exclamation", "orange")}  Внимание! Таймер тестирования</b>\n\n"
            "{timer_warning_text}\n\n"
    ...
    Window(
        Const(
            ...
            "🎥 Запиши и отправь <b>1 видео-кружок</b>"
        ),
    ...
    Window(
        Format(
            f"{emoji("❤️", "light_purple")}  <b>Второй этап отбора (Фото-/видеограф)</b>\n\n"
            "Привет! Мы очень рады, что ты откликнулся(-сь) на волонтерство МБ’26!\n\n"
    ...
    Window(
        Const(
            f"<b>Вопрос {num_emoji(3)}</b>\n\n"
            "Если у тебя есть портфолио, прикрепи ссылку, пожалуйста (Google Диск, Яндекс.Диск, Behance и т.п.).\n\n"
            f"<i>{emoji("exclamation", "orange")}  Убедись, что доступ к файлам по ссылке открыт.</i>"
        ),
    ...
    Window(
        Const(
            f"{emoji("✅")}  <b>Спасибо за твои ответы!</b>\n\n"
            "Ты успешно завершил(-а) второй этап отбора волонтеров Конференции «Менеджмент Будущего 2026».\n\n"
            f"{emoji("🕐")}  Мы вернемся с результатами с <b>4 по 7 октября 2026</b>.\n\n"
            "По всем общим вопросам обращайся в наш чат поддержки: @mbconf_support.\n"
